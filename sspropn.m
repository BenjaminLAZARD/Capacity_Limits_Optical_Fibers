function [u1, Pn] = sspropn(u0,dt,dz,nz,alpha,betap, gamma, sigma2_dz, maxiter,tol);
...
for iz = 1:nz,
	...
	u0 = u1;
    noise = sqrt(sigma2_dz/2)*(randn(nt,1) + 1i*randn(nt,1));
	u0 = u0 + noise;% Noise addition in time
 
    pn(iz)=mean(abs(noise).^2);
end
